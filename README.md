...
A fully **offline** Android build lives in [`mobile/`](mobile/) — same reader and
same judging logic, but the speech model runs **natively on the phone** (ONNX
Runtime via a Capacitor plugin), so no server and no internet are involved and
your voice never leaves the device.

Currently in **closed testing** on Google Play. It is not publicly listed yet;
testers join via the group and opt-in links.

> **بالعربية:** تطبيق أندرويد يعمل بالكامل دون إنترنت — نموذج التعرّف على الصوت
> يعمل داخل الجهاز نفسه، وصوتك لا يغادر هاتفك. التطبيق حاليًا في مرحلة الاختبار
> المغلق على جوجل بلاي.
...
cd android && ./gradlew bundleRelease     # or assembleDebug for a quick run
```

**Two large asset sets are deliberately not in this repo** (837 MB model,
381 MB audio — far past GitHub's limits). Regenerate them with the scripts in
[`tools/`](tools/) before building a release:

| Missing path | Regenerate with |
|---|---|
| `mobile/android/model_pack/src/main/assets/model/muaalem.onnx` | `tools/foldexport_4s.py` → `tools/quant_4s.py` (verify: `tools/verify_4s.py`) |
| `mobile/android/audio_pack/src/main/assets/audio/wbw/` | `tools/concat_wbw.py` |
| `mobile/www/audio/husary_timings/` *(committed)* | `tools/gen_husary_timings.py` |

The model is a 4-second, 16 kHz window (64000 samples) with mel-filterbank
extraction folded into the ONNX graph, quantised to int8 over `MatMul` only —
`ConvInteger` is not implemented by `onnxruntime-android`. Store graphics and
screenshots are generated by `tools/make_store_graphics.py` and
`tools/prep_screenshots.py`.

> ⚠️ The upload keystore (`al-muqri-release.jks`) and `key.properties` are
